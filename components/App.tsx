...
const App: React.FC = () => {
  ...
  const [biomassPulse, setBiomassPulse] = useState(false);
  const [unityPulse, setUnityPulse] = useState(false);
  const [complexityPulse, setComplexityPulse] = useState(false);
  ...
  useEffect(() => {
    ...
    if (gameState.biomass > prevResources.current.biomass) {
        setBiomassPulse(true);
        setTimeout(() => setBiomassPulse(false), 500);
    }
    if (gameState.unity > prevResources.current.unity) {
        setUnityPulse(true);
        setTimeout(() => setUnityPulse(false), 500);
    }
    if (gameState.complexity > prevResources.current.complexity) {
        setComplexityPulse(true);
        setTimeout(() => setComplexityPulse(false), 500);
    }
    if (gameState.data > prevResources.current.data) {
        setDataPulse(true);
  ...
  }, [gameState.energy, gameState.knowledge, gameState.biomass, gameState.unity, gameState.complexity, gameState.data]);
      ...
      <div className="hud-container">
          <div className="hud-top-bar">
              <div className="hud-resources hud-element">
                  ...
                  {gameState.biomass > 0 && (
                    <div className={`hud-resource-item ${biomassPulse ? 'resource-pulse' : ''}`} title="Biomass: The raw organic matter of life. Generated by life-bearing planets.">
                      <svg fill="#4ade80" viewBox="0 0 20 20"><path d="M15.042 3.958a7.5 7.5 0 00-10.608 0l-1.134 1.133a.75.75 0 000 1.061l4.243 4.242a.75.75 0 001.06 0l4.243-4.242a.75.75 0 000-1.06l-1.134-1.134zM9 8.689L5.458 5.147a6 6 0 018.486 0L11 7.623V15a.75.75 0 01-1.5 0V10a.75.75 0 00-1.06-.707L8 9.586z" /></svg>
                      <span>{Math.floor(gameState.biomass).toLocaleString()}</span>
                    </div>
                  )}
                  {gameState.unity > 0 && (
                    <div className={`hud-resource-item ${unityPulse ? 'resource-pulse' : ''}`} title="Unity: The power of collective consciousness. Generated by sentient colonies.">
                      <svg fill="#f472b6" viewBox="0 0 20 20"><path d="M10 9a3 3 0 100-6 3 3 0 000 6zM6 8a2 2 0 11-4 0 2 2 0 014 0zM1.49 15.326a.75.75 0 01-.326 1.022l-1.022.326a.75.75 0 01-.885-.885l.326-1.022a.75.75 0 011.022-.326zM17 4a2 2 0 11-4 0 2 2 0 014 0zM18.51 15.326a.75.75 0 011.022.326l.326 1.022a.75.75 0 01-.885.885l-1.022-.326a.75.75 0 01-.326-1.022zM10 12a3 3 0 100 6 3 3 0 000-6zM3.5 14a1.5 1.5 0 100 3 1.5 1.5 0 000-3zM16.5 14a1.5 1.5 0 100 3 1.5 1.5 0 000-3z" /></svg>
                      <span>{Math.floor(gameState.unity).toLocaleString()}</span>
                    </div>
                  )}
                  {gameState.complexity > 0 && (
                      <div className={`hud-resource-item ${complexityPulse ? 'resource-pulse' : ''}`} title="Complexity: The measure of intricate structures. Generated from various cosmic phenomena.">
                          <svg fill="#fb923c" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M9.25 5.25a.75.75 0 01.75-.75h.01a.75.75 0 01.75.75v.01a.75.75 0 01-.75.75h-.01a.75.75 0 01-.75-.75V5.25zM11.25 7.25a.75.75 0 01.75-.75h.01a.75.75 0 01.75.75v.01a.75.75 0 01-.75.75h-.01a.75.75 0 01-.75-.75V7.25zM9.25 9.25a.75.75 0 01.75-.75h.01a.75.75 0 01.75.75v.01a.75.75 0 01-.75.75h-.01a.75.75 0 01-.75-.75V9.25zM11.25 11.25a.75.75 0 01.75-.75h.01a.75.75 0 01.75.75v.01a.75.75 0 01-.75.75h-.01a.75.75 0 01-.75-.75v-.01zM7.25 7.25a.75.75 0 01.75-.75h.01a.75.75 0 01.75.75v.01a.75.75 0 01-.75.75h-.01a.75.75 0 01-.75-.75V7.25zM5.25 9.25a.75.75 0 01.75-.75h.01a.75.75 0 01.75.75v.01a.75.75 0 01-.75.75h-.01a.75.75 0 01-.75-.75V9.25zM7.25 11.25a.75.75 0 01.75-.75h.01a.75.75 0 01.75.75v.01a.75.75 0 01-.75.75h-.01a.75.75 0 01-.75-.75v-.01zM9.25 13.25a.75.75 0 01.75-.75h.01a.75.75 0 01.75.75v.01a.75.75 0 01-.75.75h-.01a.75.75 0 01-.75-.75v-.01z" clip-rule="evenodd" /></svg>
                          <span>{Math.floor(gameState.complexity).toLocaleString()}</span>
                      </div>
                  )}
                  {gameState.data > 0 && (
                      <div className={`hud-resource-item ${dataPulse ? 'resource-pulse' : ''}`} title="Data: Raw information collected from the universe. Used for quantum upgrades.">
                          <svg fill="#60a5fa" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M12.5 5.5a.5.5 0 01.5.5v8a.5.5 0 01-1 0v-8a.5.5 0 01.5-.5zM8.5 5.5a.5.5 0 01.5.5v8a.5.5 0 01-1 0v-8a.5.5 0 01.5-.5zM5.5 6.5a.5.5 0 00-1 0v6a.5.5 0 001 0v-6zM15.5 6.5a.5.5 0 00-1 0v6a.5.5 0 001 0v-6zM3 3.5a.5.5 0 01.5-.5h12a.5.5 0 01.5.5v12a.5.5 0 01-.5.5h-12a.5.5 0 01-.5-.5v-12z" clip-rule="evenodd" /></svg>
                          <span>{Math.floor(gameState.data).toLocaleString()}</span>
                      </div>
                  )}
              </div>
